...
const statusConfig = {
  ...
  pending: { icon: AlertTriangle, label: "Pending", className: "text-slate-300 bg-slate-700/30 border border-slate-600/30" },
};
    ...
    <div className="group rounded-2xl border border-white/10 bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl overflow-hidden transition-all duration-300 hover:border-white/20 hover:shadow-2xl shadow-xl">
      ...
      <div className="px-8 py-6 border-b border-white/5 bg-gradient-to-r from-slate-900/60 to-slate-800/60">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-semibold text-slate-100">Bug Detection Panel ({fixes.length})</h2>
        </div>
      </div>
            ...
            {isRunning ? (
              <div className="space-y-3">
                {Array.from({ length: 4 }).map((_, index) => (
                  <div key={index} className="h-10 rounded-lg bg-slate-800/60 animate-pulse" />
                ))}
              </div>
            ) : (
              <div className="rounded-xl border border-dashed border-white/10 bg-slate-900/30 p-8 text-center">
                <p className="text-sm font-medium text-slate-300">No fixes generated yet</p>
                <p className="mt-2 text-xs text-slate-500">Run an analysis to detect issues and generate automated fixes.</p>
              </div>
            )}
          </div>
        )}
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-white/5 bg-slate-900/50">
              ...
              <th className="text-left px-6 py-4 font-semibold text-xs uppercase tracking-wide text-slate-400">Line Number</th>
