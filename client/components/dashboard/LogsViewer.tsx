            ...
            isRunning ? (
              <div className="space-y-3 py-2">
                {Array.from({ length: 5 }).map((_, index) => (
                  <div key={index} className="h-8 rounded-md bg-slate-800/60 animate-pulse" />
                ))}
              </div>
            ) : (
              <div className="text-slate-500 text-center py-8">No logs yet. Start an agent run to stream execution logs.</div>
            )
          ) : (
            filtered.map((log) => (
